...
export function SpectrumHistory({ walletAddress }: { walletAddress: string }) {
  ...
  return <section className="glass rounded-[24px] p-5 mt-5 flex flex-col gap-4" aria-label="Sent Spectrums">
    ...
    {entries.map(e => <div key={e.depositId} className="glass-sm rounded-xl p-4 flex flex-col gap-2">
      ...
      <div className="text-xs flex items-center gap-2">{spectrumConfigured && closed[e.depositId] === undefined ? <DataSkeleton className="w-28 h-3" label="Checking bundle status" /> : <span className="opacity-55">{!spectrumConfigured || closed[e.depositId] === null ? 'Status unavailable' : closed[e.depositId] ? 'Claimed or cancelled' : 'Unclaimed'}</span>}<span className="opacity-55">· {new Date(e.createdAt).toLocaleDateString()}</span></div>
